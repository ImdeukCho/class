...
//		 List<Dept> list = session.selectList("selectAll2"); // Mapper의 sql문의 id가 들어감
	...
	public List<Dept> selectDynamicDeptno(SqlSession session, HashMap<String, Integer> map) {
		List<Dept> list = session.selectList("com.dept.DeptMapper2.selectDynamicDeptno", map); // 동적실행이므로 전체문 출력도 할수있게 selectList 사용한다.
		return list;
	}

	public List<Dept> selectDynamicChoose(SqlSession session, HashMap<String, String> map) {
		List<Dept> list = session.selectList("com.dept.DeptMapper3.selectDynamicChoose", map); // 동적실행이므로 전체문 출력도 할수있게 selectList 사용한다.
		return list;
	}
