...
public class OracleTXService{
   ...
   public void insertDeltet(Dept xx, int i) {
	   Connection con = null;
	   ArrayList<Dept> list = null;
		try {
			con = DriverManager.getConnection(url, userid, passwd);
	con.setAutoCommit(false);  // 자동 commit 중지
			dao.insert(con, xx);   // insert 정상 실행
			dao.delete(con, i);    // exception 발생
	con.commit();
		} catch (SQLException e) {
			if(con != null) {
				try {
					System.out.println("insertDeltet catch RollBack=================");
	con.rollback();
